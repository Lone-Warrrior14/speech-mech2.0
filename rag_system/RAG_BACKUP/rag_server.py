...
try:
    log_boot("[BOOT] Linking rag_answer...")
    import rag_answer
    log_boot("[BOOT] Linking universal_reader...")
    import universal_reader
    log_boot("[BOOT] Linking insert_pinecone...")
    ...
    def run_upload():
        ...
        if results:
            ...
            for path, text in results:
                ...
                try:
                    if os.path.exists(path): shutil.copy(path, dest_path)
                    else:
                        with open(dest_path + ".txt", "w", encoding="utf-8") as f:
                            f.write(text)
                except: pass
                insert_document(os.path.basename(path), text, namespace)
            print(f"[RAG-SERVER] Uplink complete for {namespace}")
