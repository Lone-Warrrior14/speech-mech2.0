...
while True:
    ...
    if authenticated_user:
        ...
        if len(matches) == 0:
            ...
            if reg_choice and ("yes" in reg_choice or "register" in reg_choice):
                ...
                speak(f"User {new_user} created. Please type your new password in the terminal. This is compulsory.")
                new_password = ""
                while not new_password:
                    new_password = getpass.getpass("Create Password: ")
                    if not new_password:
                        speak("Password cannot be empty. Please try again.")
                        print("Password cannot be empty.")
                
                set_user_password(new_user, new_password)
                speak("Password created successfully. Would you also like to register your face for easier login? Say yes or no.")
                
                method = listen()
                if method and ("yes" in method or "face" in method or "register" in method):
                    speak("Starting face registration.")
                    register_face(new_user)
                else:
                    speak("Skipping face registration.")
    ...
    while True:
        ...
        if "stop chat" in command:
            speak("Stopping chat mode. Going to sleep.")
            break
            
        if "stop rag" in command or "stop r a g" in command or "stop document" in command:
            if interaction_mode == "rag":
                interaction_mode = "chat"
                speak("Exiting RAG mode. Returning to normal chat.")
            else:
